...
class GenerateStep(PipelineStep):
    ...
    def _run_plan_parallel(self, plan, train, real, constants, config, out_dir, n_rows,
        ...
        A run that hits its own SIGALRM timeout (see _time_limit) is
        caught inside _run_one itself, same as the sequential path -- it
        comes back as a normal 'failed' record, not a raised exception,
        so it can never hang this loop or the pool. As long as at least
        one run in the whole plan succeeds, GenerateStep.run() returns
        normally afterward and run_pipeline() proceeds to the next step
        (evaluate, ...) -- a stuck/timed-out AIM run costs at most its
        own timeout, never the rest of the campaign."""
